"""
This is taken directly from https://github.com/stanislavkozlovski/Red-Black-Tree with a few modifications.
The changes made allow the tree to use duplicate values and allow for efficient sampling of the list.
There is a bug right now, but a TODO item is to make the sample by top-k items work.
"""
...
__all__ = [ 'NodeData', 'RedBlackTree', 'Node' ]
...
class NodeData:
    ...
    def __init__( self, data ):
        self.data = data
        self.count = NodeData.n_node_datum
        NodeData.n_node_datum += 1
    ...
    def __eq__( self, other ):
        if( isinstance( other, NodeData ) ):
            return ( self.data == other.data ) and ( self.count == other.count )
        return self.data == other
    ...
    def __str__( self ):
        return 'Datum( %d, %d )'%( self.data, self.count )
...
class Node:
    def __init__(self, value, color, parent, left=None, right=None):
        ...
        self.nodes_to_left = 0
        self.nodes_to_right = 0
        ...
        if( left is not None ):
            if( left.color == NIL ):
                ...
                self.left_value_sum = left.total_value
                self.nodes_to_left  = left.total_count
        ...
        if( right is not None ):
            if( right.color == NIL ):
                ...
                self.right_value_sum = right.total_value
                self.nodes_to_right  = right.total_count
    ...
    def left( self, node ):
        ...
        if( node.color == NIL ):
            ...
            self.left_value_sum = node.total_value
            self.nodes_to_left  = node.total_count
    ...
    def right( self, node ):
        ...
        if( node.color == NIL ):
            ...
            self.right_value_sum = node.total_value
            self.nodes_to_right  = node.total_count
    ...
    def total_value( self ):
        if( self.color == NIL ):
            return 0
        return self.value + self.left_value_sum + self.right_value_sum

    @property
    def total_count( self ):
        if( self.color == NIL ):
            return 0
        return 1 + self.nodes_to_left + self.nodes_to_right
...
class RedBlackTree:
    ...
    def __init__(self):
        ...
        self.total_value_in_tree = 0

    def sample_from_top_k( self, k ):
        assert 0, 'Not implemented.  There is a bug somewhere with the value attrs in Node'
    ...
    def __reversed__(self):
        if not self.root:
            return list()
        yield from self.root.__reversed__()

    def add(self, value):
        self.total_value_in_tree += value
        if not self.root:
    ...
    def remove(self, value):
        ...
        if node_to_remove is None:  # node is not in the tree
            assert 0, 'Remove failed!  value: %s %s'%( str( value ), type( value ) )
            return
        self.total_value_in_tree -= value
    ...
    def _right_rotation(self, node, parent, grandfather, to_recolor=False):
        ...
        # Need to update these in the correct order in order for the node stats to update correctly

        # Update the left right connections
